...
#
# -shm и -journal НЕ копируем и остатки от прошлого раза удаляем. -shm — разделяемая
# память живой базы, к чужому снимку она не подходит. -journal оставляет уже САМО
# приложение: оно открывает копию на запись, чтобы накатить неприменённый -wal, и рядом
# остаётся откатный журнал. Пустой безвреден, но попасться он может и непустым, а
# непустой журнал рядом со свежескопированной базой SQLite вправе откатить — то есть
# вернуть в снимок страницы от прошлого раза. Ровно та беда, от которой бережёмся с -wal.
copy_db() {
  tag=$1; src=$2; uid=$3
  CHANGED=0
  [ -f "$src" ] || return 1
  ...
  base=$(basename "$src")
  rm -f "$dst/$base-shm" "$dst/$base-journal" 2>/dev/null
  ...
  # Владельца и метку ставим ВСЕГДА, даже если ничего не переписывали. Приложение могли
  # переустановить: uid и категории SELinux у него тогда новые, а копия осталась прежней —
  # и без этого он не открыл бы собственный файл до первого изменения у источника.
  # Это правка inode, а не данных: в ОЗУ она бесплатна.
...
# Копии, которым больше нечего отражать: мессенджер снесли, или метка исчезла из DBS
# после обновления модуля. В ОЗУ такая копия висела бы до перезагрузки — Telegram это
# 156 МБ памяти за приложение, которого на телефоне уже нет.
drop_orphans() {
  for _d in "$DIR"/*; do
    [ -d "$_d" ] || continue
    _t=$(basename "$_d")
    ...
    if [ -z "$_src" ]; then
      rm -rf "$_d" 2>/dev/null; log "копия $_t: такой базы модуль больше не знает — убрана"
    elif [ ! -f "$_src" ]; then
      rm -rf "$_d" 2>/dev/null
      rm -f "$MODDIR/.src-$_t" "$MODDIR/.src-$_t-wal" 2>/dev/null
      log "копия $_t: мессенджер не установлен — убрана"
    fi
  done
}
...
sync_all() {
  ...
  # Осиротевшие копии убираем на обычном прогоне: адресный запрос приходит, когда клиент
  # ждёт ответа, и лишней работы там быть не должно.
  [ -n "$want" ] || drop_orphans
...
while :; do
  if ! app_uid >/dev/null; then
    # После перезагрузки раздел приложения зашифрован до первой разблокировки, и stat
    # по его папке не проходит. Переспрашивать раз в минуту нельзя: приложение
    # подключается сразу после разблокировки, и если копий ещё нет, оно откладывает
    # импорт. 15.09 так и вышло — мост опоздал на 44 секунды. Спрашиваем часто, а в
    # журнал пишем один раз: иначе строка полезет по двенадцать раз в минуту.
    [ "$locked" = "0" ] && log "данные приложения ещё недоступны — ждём разблокировки"
    locked=$((locked+1))
    sleep 5
    continue
  fi
  [ "$locked" = "0" ] || log "данные приложения доступны"
  locked=0
