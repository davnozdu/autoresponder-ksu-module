...
# Приложение сначала пробует setStreamVolume(VOICE_CALL,0) — но на этом телефоне это НЕ
# глушит приём в наушник во время активного звонка (живой тест, dumpsys audio показывал
# Muted:false, звук всё равно был слышен): во время звонка "TFA Profile" (usecase RX)
# переключается на "receiver" через отдельный smart-усилитель NXP TFA98xx, а не через
# обычный кодек-DAC — обычный Android-мьют стрима его не трогает. У чипа свой
# mute-переключатель "TFA Mute" (проверено живым звонком: tinymix во время разговора
# реально глушит звук в трубке). Контрол-номер 294 — на этой прошивке tinymix не умеет
# обращаться по имени с пробелом, только по номеру; если после обновления OxygenOS звук
# снова начнёт быть слышен, найти новый номер: `tinymix | grep -i "TFA Mute"`.
# ВАЖНО: запись в контрол принимается только во время АКТИВНОЙ голосовой сессии — в
# простое tinymix отрабатывает без ошибки, но значение молча не применяется (тоже
# проверено живым тестом), поэтому muteout шлётся приложением уже ПОСЛЕ answerCall().
TFA_MUTE_CTL=294
muteout() { # on|off
  val=unmute; [ "$1" = "on" ] && val=mute
  tinymix "$TFA_MUTE_CTL" "$val" >/dev/null 2>&1
  log "muteout $1 (tinymix $TFA_MUTE_CTL $val)"
  reply ok "muteout:$1"
}
